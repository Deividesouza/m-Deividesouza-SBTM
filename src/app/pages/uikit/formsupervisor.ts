...
@Component({
    ...
    templateUrl: './formsupervisor.componente.html',

    providers: [MessageService, ConfirmationService, DialogService, provideNgxMask()]

})
export class FormSupervisor implements OnInit {
...
*/
        ...
        this.http.get<Product[]>(`${environment.url}/pessoas/fisicas`).subscribe(
            (data) => {
                ...
                const filteredData = data.filter(product =>
                    product.perfilAcesso.id === 1 || product.perfilAcesso.id === 4 || product.perfilAcesso.id === 5
                );
    ...
    onNovo() {
        this.router.navigate(['/uikit/formsupervisorcadastro']);
    }
