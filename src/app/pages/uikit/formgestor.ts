...
@Component({
    ...
    templateUrl: './formgestor.componente.html',

    providers: [MessageService, ConfirmationService, DialogService, provideNgxMask()]

})
export class FormGestor implements OnInit {
...
*/
        ...
        this.http.get<Product[]>(`${environment.url}/pessoas/fisicas`).subscribe(
            (data) => {
                ...
                const filteredData = data.filter(product =>
                    product.perfilAcesso.id === 1 || product.perfilAcesso.id === 4 || product.perfilAcesso.id === 5 || product.perfilAcesso.id === 6
                );
    ...
    onNovo() {
        this.router.navigate(['/uikit/formgestorcadastro']);
    }
