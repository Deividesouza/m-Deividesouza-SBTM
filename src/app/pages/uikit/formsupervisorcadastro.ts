...
@Component({
    selector: 'app-formsupervisorcadastro',
    standalone: true,
    ...
    templateUrl:'formsupervisorcadastro.componente.html' ,


})
export class FormSupervisorCadastro implements OnInit {
        ...
        this.dropdownPerfil.getPerfil().subscribe({
            next: (data: any[]) => {
                this.vardropdownPerfis = data
                    .filter(perfil => perfil.id === 1 || perfil.id === 4 || perfil.id === 5) // [Participante 1, Operador 4, Supervisor 5] pelos IDs desejados
                    .map((perfil) => ({
        ...
        this.dropdownTipos.getTipos().subscribe({
            next: (data: any[]) => {
                ...
                this.vardropdownTipos = data
                    .filter(tipo => [1, 4, 5].includes(tipo.id)) //  [Participante 1, Operador 4, Supervisor 5] pelos IDs desejados
                    .map((tipos) => ({
    ...
    onVoltar() {
        this.router.navigate(['/uikit/formsupervisor']);
    }
    ...
    onSubmit() {
        if (this.formulario.valid) {
            ...
            // Validação do perfil
        if (this.selectedPerfil?.code !== 1 && this.selectedPerfil?.code !== 4 && this.selectedPerfil?.code !== 5) {
            this.messageService.add({
                severity: 'error',
                summary: 'Erro',
